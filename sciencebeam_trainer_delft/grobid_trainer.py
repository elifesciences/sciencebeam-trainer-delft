...
# from delft.utilities.cloud_support import patch_cloud_support
...
def train(
        model, embeddings_name, architecture='BidLSTM_CRF', use_ELMo=False, input_path=None, output_path=None,
...
if __name__ == "__main__":
    ...
    kwargs = dict(batch_size=1, max_sequence_length=50, max_epoch=1)

    if action == 'train':
        train(
            model, embeddings_name, architecture=architecture, use_ELMo=use_ELMo, input_path=input_path, output_path=output,
            **kwargs
        )
    ...
    if action == 'train_eval':
        ...
        train_eval(
            model, embeddings_name, architecture=architecture, use_ELMo=use_ELMo, input_path=input_path, output_path=output, fold_count=args.fold_count,
            **kwargs
        )
